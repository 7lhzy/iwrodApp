...
//                "interpret text, sentorig text, senttrans text)");
        String[] columns=new String[]{"word",
                "pse","prone","psa","prona","interpret","sentorig","senttrans","sent"};
        ...
        while(cursor.moveToNext()){
            ...
            w=new WordValue(strArray[0],strArray[1],strArray[2],strArray[3],strArray[4],strArray[5],strArray[6],strArray[7],strArray[8]);
        }
        cursor.close();
        return w;
    }
    ...
    public WordValue getWordFromInternet(String searchedWord){
        ...
        String str=null;
        try{
            String tempUrl= HttpUtil.url1+tempWord+HttpUtil.url2;
            Log.d("url",tempUrl);
            in=HttpUtil.getInputStreamByUrl(tempUrl);
            if(in!=null){
